    ...
    <div class="rounded-xl border border-neutral-800 bg-neutral-900/50 p-4">
      ...
      <div class="flex items-center justify-between mb-3">
        <div class="flex items-center gap-2">
          <ClipboardCheck class="w-4 h-4 text-neutral-400" />
      ...
      <div class="space-y-2">
        ...
        >
          <button
            class="w-[18px] h-[18px] rounded border-[1.5px] border-neutral-600 shrink-0 transition-colors hover:border-neutral-400 active:border-green-500"
            @click="$emit('toggle', item.id)"
          />
          <span class="text-sm text-neutral-300 flex-1 leading-snug">{{ item.content }}</span>
        </div>
      </div>

      <!-- More count -->
      <p v-if="pendingItems.length > 3" class="text-xs text-neutral-500 mt-2 pl-[30px]">
